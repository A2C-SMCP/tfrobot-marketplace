...
| 文档 | 范围 |
| --- | --- |
| **[SKILL 协议 v1](protocol-v1.md)** ⭐ | SKILL 内容契约（frontmatter / 目录 / `.skillenv` / 占位符 / `skills` 工具 / 安全模型）+ Robot 多来源汇聚视角 |
...
* **Marketplace 仓库**——Git 仓库分发，详见 [Marketplace v1 规范](../marketplace/protocol-v1.md)。

不同来源带来不同的执行能力，但**落到加载方手中的都是等价的 SKILL 文件夹**——内容契约完全一致。

## 子任务产出（设计史）

A1\~A4 是协议规范的分步设计文档；如与协议 v1 表述冲突，**以协议 v1 为准**。保留作为决策 rationale 与设计史，统一收录于 [设计史索引](design-history/index.md)。
...
| A3 `runtime` 枚举 | [TFRS-185](https://turingfocus.atlassian.net/browse/TFRS-185) | 已完成（v1 撤回） | [runtime 枚举](design-history/runtime-enum.md) |
