...
本协议**不另行设计** MCP 配置 schema：完整 schema 来自 A2C-SMCP；本协议**只规定** Plugin 内的目录约定 + 摘要当前对齐版本（v0.2.0）的字段速查，便于 Plugin 作者撰写时无需切换文档。
...
* **同步策略**：A2C-SMCP 升级到 v0.3+ 时，本节摘要随其变更同步；以 A2C-SMCP 为准
